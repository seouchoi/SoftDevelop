...
def homepage():
    ...
    if 'key_id' in session: #session이 True인지 확인(로그인상태인지 아닌지)
        ...
        invite_data = invite_db_handler.get_received_invites(key_id)
        print(invite_data)
        return render_template("login_homepage.html", contest_data = random_contest_data, invite_data = invite_data) #홈페이지(로그인 상태)
    else:
